...
> 本公开 GitHub 仓库收录已获用户明确确认；卓叔计划将作为 v1 `Built-in Program`。正式发行包在授权 gate 通过时可包含运行时派生制品，以及固定路径和 digest 的原始训练日志 XLSX；原始 DOCX 与任意其他 Office 文件不进入安装包。授权必须覆盖具体制品、修改、署名及实际分发渠道。
