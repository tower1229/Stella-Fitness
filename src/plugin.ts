...
export function registerStellaFitnessPlugin(
  ...
  const resolveWorkoutLogConfirmationText = async (input: {
    ...
    if (turn.status === "ambiguous") {
      if (
        turn.classificationFailure !== undefined &&
        turn.pendingFieldCount !== undefined
      ) {
        api.logger?.warn(
          `stella-fitness confirmation classification unresolved: reason=${turn.classificationFailure} pendingFieldCount=${turn.pendingFieldCount}`,
        );
      }
      return "我没能确定你要确认或修改哪些字段，因此没有保存。请回复“确认”，或直接说明字段和值。";
    }
  ...
  const processWorkoutLogImage = async (input: {
    readonly event: PluginHookInboundClaimEvent;
    readonly sessionKey?: string;
    readonly runId?: string;
  }): Promise<string | undefined> => {
    const upload = await workoutLogUpload(input.event);
    if (upload === undefined) return undefined;
    const request = {
      intent: workoutLogIntent(input.event),
      runId:
        input.runId ??
        input.event.runId ??
    ...
    const correctionId = workoutLogCorrectionId(input.event);
    ...
    try {
      ...
      if (result.status === "confirmation" && input.sessionKey !== undefined) {
        await workoutLogConfirmation.bind({
          sessionKey: input.sessionKey,
          confirmationId: result.confirmationId,
          issuedAt: result.artifact.provenance.receivedAt,
          ...(input.event.messageId === undefined
            ? {}
            : { messageId: input.event.messageId }),
        });
      }
      return formatWorkoutLogResult(result);
    } catch (error) {
      if (error instanceof MultiSessionWorkoutLogPageError) {
        return "这张照片包含多次训练。请裁剪到只保留一次训练记录后重新发送。";
      }
      throw error;
    }
  };
  ...
  api.on(
    ...
    async (event, context) => {
      ...
      const replyText = await processWorkoutLogImage({
        event,
  ...
  const handleDedicatedTextInput = async (input: {
    ...
    readonly source: {
      ...
      const localDate = confirmedFitnessLocalDate(
        receivedAt,
        currentOpenClawConfig(api).agents?.defaults?.userTimezone,
      );

      const intent = await arbitrateTextInput({
        text,
        receivedAt,
        localDate,
        isInitial12RMText,
        ...
        classifier: dedicatedFitnessQueryClassifier,
      });
      ...
      switch (intent.kind) {
        ...
        case "help-or-usage": {
          let status: Awaited<ReturnType<StellaFitnessRuntime["programJourneyStatus"]>> | undefined;
          try {
            status = await stellaRuntime.programJourneyStatus({ date });
          } catch {
            status = undefined;
          }
          return { text: formatContextualHelp(status) };
        }
        case "activation": {
          const status = await stellaRuntime.programJourneyStatus({ date });
          if (status.state !== "READY_TO_ACTIVATE") {
            return { text: formatJourneyStatus(status, date) };
          }
          if (intent.intent.kind === "clarification") {
            return { text: intent.intent.message };
          }
          if (intent.intent.kind === "defer") {
            return { text: "好的，暂不开始。准备好后告诉我“本周开始”或“下周开始”即可。" };
          }
          return {
            text: await activateProgramReply(stellaRuntime, intent.intent.cycleStart),
          };
        }
        case "prerequisite-ack": {
          const status = await stellaRuntime.acknowledgePrerequisite({
            prerequisiteId: intent.prerequisiteId,
        ...
        case "deterministic-fitness-query":
          return;
        case "timezone-required":
          return {
            text: "请先确认你的 IANA 时区，我才能确定“今天”和“本周”的日期边界。",
          };
        case "exact-facts-query":
          return {
            text: await formatAvailableProgramFacts(stellaRuntime, intent.query),
          };
        case "out-of-scope-advice": {
        ...
        case "body-weight-candidate": {
          if (intent.correctionId === undefined) {
            const journeyStatus = await stellaRuntime.programJourneyStatus({ date });
            if (
          ...
          const result = await stellaRuntime.correctBodyWeight({
            replacesObservationId: intent.correctionId,
            text,
            receivedAt,
            source,
          });
  ...
  api.on(
    ...
    async (event, context) => {
      ...
      const dashboardMedia = dashboardMediaEvent({
        text: event.cleanedBody,
        stateDirectory: api.runtime.state.resolveStateDir(process.env),
        channel: context.messageProvider ?? "dashboard",
        ...(context.runId === undefined ? {} : { runId: context.runId }),
      });
      if (dashboardMedia !== undefined) {
        const replyText = await processWorkoutLogImage({
          event: dashboardMedia,
          ...(context.sessionKey === undefined
            ? {}
            : { sessionKey: context.sessionKey }),
          ...(context.runId === undefined ? {} : { runId: context.runId }),
...
function dashboardMediaEvent(input: {
  readonly text: string;
  readonly stateDirectory: string;
  readonly channel: string;
  readonly runId?: string;
}): PluginHookInboundClaimEvent | undefined {
  const match = /\[media attached(?:\s+\d+\/\d+)?:\s*media:\/\/inbound\/([^\]\r\n]+?)\s+\((image\/(?:jpeg|png|webp))\)\]/iu.exec(
    input.text,
  );
  ...
  return {
    ...
    metadata: {
      mediaPath: join(input.stateDirectory, "media", "inbound", fileName),
...
function formatRemainingWorkoutLogConfirmation(result: {
  ...
  if (result.acceptedCount === 0) {
    return [
      `这些字段没有可确认的识别值：${result.fields.map((field) =>
